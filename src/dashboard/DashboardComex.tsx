...
import {
  ...
  ShieldAlert, HeartPulse, GraduationCap, Target, Activity, Star,
} from 'lucide-react';
import {
...
export default function DashboardComex({ companyId }: Props) {
  ...
  const [hAn,       setHAn]       = useState(1607);
  ...
  async function charger() {
    ...
    if (r8.data) { setEffectif(r8.data.effectif ?? 50); setHAn(r8.data.h_an ?? 1607); }
    setAccidents(r1.data ?? []);
    setActions(r2.data ?? []);
    setHabs(r3.data ?? []);
    setRisques(r4.data ?? []);
    setNcs(r5.data ?? []);
  ...
  const kpis = useMemo(() => {
    ...
    if (ncOuvertes.length > 0)   alertes.push({ level: 'amber', msg: `${ncOuvertes.length} non-conformité(s) ouverte(s)`,                path: '/dashboard/audits' });
    ...
    return { accArret: accArret.length, jours, TF, TG, actRetard: actRetard.length, actTerminees: actTerminees.length, tauxPDCA, totalActions: actions.length, habsPerimees: habsPerimees.length, habsBientot: habsBientot.length, risquesCrit: risquesCrit.length, ncOuvertes: ncOuvertes.length, tauxNC, moyenneSat, scoreGlobal, scoreSecurite, scoreHabs, scoreMaitrise, scoreAudit, accChart, radarData, alertes };
  }, [accidents, actions, habs, risques, ncs, audits, sat, effectif, hAn]);
  ...
  const ttStyle = { contentStyle: { background: '#1e293b', border: '1px solid #334155', borderRadius: 8, fontSize: 11 } };
            ...
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={kpis.accChart}>
                  ...
                  <XAxis dataKey="mois" stroke="#e2e8f0" tick={{ fill: '#94a3b8', fontSize: 11 }} />
                  <YAxis stroke="#e2e8f0" tick={{ fill: '#94a3b8', fontSize: 10 }} allowDecimals={false} />
                  <Tooltip {...ttStyle} />
