# SMI Dashboard Phase 2 — Partie 1 : Migration SQL

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Créer les 5 tables Supabase pour les modules Phase 2 avec company_id + RLS.

**Architecture:** Chaque table reçoit `company_id uuid NOT NULL REFERENCES companies(id)`. La RLS utilise `get_user_company_id()` déjà définie en Phase 1. Les noms de tables correspondent à ceux du source qhse-dashboard2 sauf `registre_duerp` → `risques` et `plan_actions` → `actions`.

**Tech Stack:** Supabase PostgreSQL, RLS via get_user_company_id()

---

### Task 0 : Migration SQL — 5 tables dashboard

**Files:**
- Create: `supabase/migrations/20260603000001_dashboard_modules.sql`

- [ ] **Step 1 : Créer le fichier de migration**

```sql
-- supabase/migrations/20260603000001_dashboard_modules.sql
...
- [ ] **Step 2 : Appliquer la migration Supabase**

```powershell
# Depuis le dossier site-internet-mase
npx supabase db push
```

Résultat attendu : `Applying migration 20260603000001_dashboard_modules.sql... done`

Si erreur "already exists" : la migration a déjà été appliquée, continuer.

- [ ] **Step 3 : Vérifier les tables dans Supabase**

Aller sur https://supabase.com/dashboard/project/ulceeurwibmbtnqhkaao/editor et vérifier que les 5 tables apparaissent : `risques`, `actions`, `accidents`, `habilitations`, `kpi_objectifs`.

- [ ] **Step 4 : Commit**

```powershell
git add supabase/migrations/20260603000001_dashboard_modules.sql
git commit -m "feat(db): add 5 dashboard module tables with company_id RLS"
```

---

**Fin Partie 1.** Continuer avec la Partie 2 (utils + CSS).
