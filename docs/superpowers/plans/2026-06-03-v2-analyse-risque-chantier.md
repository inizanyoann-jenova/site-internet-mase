# Analyse Risque Chantier (Mobile) — Plan d'implémentation V2

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ajouter un module d'analyse des risques chantier optimisé pour une utilisation sur smartphone, permettant aux opérateurs de saisir une analyse de risque sur le terrain avant de démarrer une intervention.

**Architecture:** Table `risques_chantier` (distincte du DUERP — risques ponctuels de chantier vs. risques permanents). Composant React avec design mobile-first : grands boutons tactiles, formulaire simplifié en étapes, notation rapide par icônes. Ajouté dans le groupe "Sécurité" de la sidebar.

**Tech Stack:** React 19 + TypeScript, Supabase PostgreSQL, Tailwind CSS (mobile-first), classes `.db-*`
...
**Files :**
- Create : `supabase/migrations/20260604000010_risques_chantier.sql`
...
create table if not exists risques_chantier (
  ...
  heure_debut      text,
  -- Risques identifiés (stockés en JSON pour flexibilité)
...
```
Résultat attendu : `Applying migration 20260604000010_risques_chantier.sql... done`

- [ ] **Step 3 : Commit**

```powershell
git add supabase/migrations/20260604000010_risques_chantier.sql
git commit -m "feat(v2): migration SQL risques chantier"
```

---

### Task 1 : Composant AnalyseRisqueChantier

**Files :**
- Create : `src/dashboard/AnalyseRisqueChantier.tsx`
...
```powershell
git add src/dashboard/AnalyseRisqueChantier.tsx
git commit -m "feat(v2): composant AnalyseRisqueChantier (mobile-first, saisie terrain)"
```
...
**Files :**
- Create : `src/pages/DashboardRisqueChantierPage.tsx`
- Modify : `src/main.tsx`
...
```tsx
// src/pages/DashboardRisqueChantierPage.tsx
...
**Note :** Le rôle `operateur` peut saisir une analyse (canWrite = true) car c'est précisément leur cas d'usage terrain.
...
Dans le groupe `Sécurité`, ajouter après "Plan d'actions" :
```tsx
...
```powershell
git add src/pages/DashboardRisqueChantierPage.tsx src/main.tsx src/components/dashboard/DashboardSidebar.tsx
git commit -m "feat(v2): route /dashboard/risque-chantier + sidebar (mobile-first)"
```
